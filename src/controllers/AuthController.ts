...
export const AuthController = {
    login: async (req: Request, res: Response): Promise<void> => {
        ...
        if (admin && await bcrypt.compare(contrasena, admin.adminContrasena)) {
            res.status(200).json({ mensaje: "Login exitoso", rol: "Administrador", id: admin.adminCodAdministrador });
            return;
        }
        ...
        if (empleado && await bcrypt.compare(contrasena, empleado.emplContrasena)) {
            res.status(200).json({ mensaje: "Login exitoso", rol: "Empleado", id: empleado.emplCodEmpleado });
            return;
        }
        ...
        if (cliente && await bcrypt.compare(contrasena, cliente.clContrasena)) {
            res.status(200).json({ mensaje: "Login exitoso", rol: "Cliente", id: cliente.clCodCliente });
            return;
        }
